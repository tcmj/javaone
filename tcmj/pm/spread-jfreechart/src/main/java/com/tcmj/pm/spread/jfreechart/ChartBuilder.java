...
//        SpreadPeriod[] periods = cfinder.getPeriods();

        TaskSeries localTaskSeries1 = new TaskSeries("OutputBarList");

        int no = 0;
        for (SpreadPeriod bar : periods) {
            no++;
            Task task = new MyTask(bar);
...
//---------------------------------------------------------------------
    ...
    public PeriodAxis createTimeAxis() {
        ...
        Date sd = new Date(periods[0].getStartMillis());
        Date ed = new Date(periods[periods.length - 1].getStartMillis());

        long days = daysbetween(sd, ed);
        ...
        if (days <= 31) { //2m
            ...
            timeaxis.setAutoRangeTimePeriodClass(Week.class);
...
//            arrayOfPeriodAxisLabelInfo[0] = new PeriodAxisLabelInfo(Hour.class, new SimpleDateFormat("HH"));
            ...
            arrayOfPeriodAxisLabelInfo[1] = new PeriodAxisLabelInfo(Week.class, new SimpleDateFormat("w"));
