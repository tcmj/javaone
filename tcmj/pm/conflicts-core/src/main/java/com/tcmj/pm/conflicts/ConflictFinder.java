...
/**
 * Finds conflicts on time periods.
 * <pre>
 * Start C = 0%                  P={};
 * A1:   C = 0% + 30% = 30%;     P = {P1}         no conflict
 * A4:   C = 30% + 50% = 80%;    P = {P1,P4}      no conflict
 * A2:   C = 80% + 40% = 120%;   P = {P1,P4,P2}   conflict
 * E1:   C = 120% - 30% = 90%;   P = {P4,P2}      no conflict
 * E4:   C = 90% - 50% = 40%;    P = {P2}         no conflict
 * A3:   C = 40% + 20% = 60%;    P = {P2,P3}      no conflict
 * E2:   C = 60% - 40% = 20%;    P = {P3}         no conflict
 * E3:   C = 20% - 20% = 0%;     P = {}           no conflict
 * ENDE
 * </pre>
 * Created:     2009/03/03 by TDEUT
 * Last Modify: 2009/04/30 by TDEUT
 * Last Modify: 2015/10/14 by TDEUT
 * @author tdeut Thomas Deutsch
 * @version 1.10
 */
public class ConflictFinder<T extends Bar> {

    /** slf4j Logging framework. */
    private static final transient Logger LOG = LoggerFactory.getLogger(ConflictFinder.class);
     ...
     * @param list a list of classes which implements the Bar interface
     */
    public void addToInputList(Collection<T> list) {
        if (list == null) {
     ...
     */
    public void addToInputList(T bar) {
        if (inputList == null) {
            inputList = new ArrayList<>();
        }
        inputList.add(bar);
    ...
    public void calculate() {
        if (inputList == null || inputList.isEmpty()) {
            ...
            LOG.debug("Start computing {} input items with max allowed weight of {} (Precision={})",
                    inputList.size(), String.format("%3.0f%%", precision.long2Double(getMaxAllowedWeightAsLong()) * 100), getPrecision());
    ...
    private void buildSortedDatesList() {
        ...
        LOG.debug("Sorted dates List: {}", sortedList);
    }
    ...
    private void buildConflictList() {
        ...
        outputConflictList = new ArrayList<>();
        outputBarListAll = new ArrayList<>();

        //init list to hold causing bars
        List<Bar> tempBarList = new ArrayList<>();
        ...
        for (ListIterator<SortItem> it = sortedList.listIterator(); it.hasNext(); ) {
            ...
            //final boolean hasDuration = (counter > 0 && (sortItem.getDate().compareTo(tempnext.getDate()) != 0));
            //check if there is a duration:
            final boolean hasDuration = (counter > 0 && (sortItem.getDate().compareTo(tempnext != null ? tempnext.getDate() : null) != 0));
            ...
            if (hasDuration) { //true means: start (at least) creation of a bar
                ...
                OutputBar nbar = new OutputBar(newkey, sortItem.getDate(), tempnext.getDate(), dblCounter, isConflict);
...
//        final String formatString = "[{0}] : {1}   ({2} {3} {4})= {5} : {6}  End: {7}   Bars: {8}";
        ...
        if (item != null) {
            ...
            if (item.getType() == SortItemType.STARTITEM) {
                data[3] = String.format("%3.0f", precision.long2Double((counter - item.getWeightAsLong())) * 100);
                data[4] = "+";
            } else {
                data[3] = String.format("%3.0f",  precision.long2Double((counter + item.getWeightAsLong())) * 100);
                data[4] = "-";
            ...
            data[5] = String.format("%3.0f", precision.long2Double((item.getWeightAsLong())) * 100);
        }
        data[6] = String.format("%3.0f%%",  precision.long2Double(counter) * 100);
    ...
    public Date getMinStartDate() {
        ...
        if (this.sortedList == null) {
            final List<SortItem> tempList = new ArrayList<>();
            for (Bar bar : inputList) {
                SortItem sitemA = new SortItem(SortItemType.STARTITEM, precision.double2Long(bar.getWeight()), bar);
                tempList.add(sitemA);
            }
     ...
     * @return list of Bar items
     */
    public List<? extends Bar> getInputList() {
        return this.inputList;
    }

    /**
     * conflict calculation name.
     * @return a name to set for this calculation
     */
    ...
    /**
     * conflict calculation name.
     * @param name a name to set for this calculation
     */
